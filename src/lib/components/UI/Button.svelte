...
<style lang="scss">
  ...
  a {
    ...
    color: white;
    border-radius: 5px;
    box-shadow: 1px 1px 3px $anchor-box-shadow;
    cursor: pointer;
  ...
  a:active {
    ...
    box-shadow: 1px 1px 8px $anchor-active-box-shadow;
  }
...
{#if href}
  <a {href}><slot/></a>
{:else}
  <button class="{mode} {color}" {type} on:click {disabled} style="width: {width}{width === 'auto'? '': '%'}">
    <slot/>
  </button>
{/if}
