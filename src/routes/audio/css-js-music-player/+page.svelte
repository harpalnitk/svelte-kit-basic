...
let playPauseHtml = ' <i class="fas fa-play"></i>';
    ...
    // NEXT PREV
    const nextSong = () => {
        console.log('next data.length', audioData.length);
		if (trackIndex === audioData.length - 1) {
			trackIndex = 0;
		} else {
			trackIndex += 1;
		}
        console.log('next song track index',trackIndex);
        loadTrack();
		playPauseAudio();
	};

	const prevSong = () => {
        console.log('prev data.length', audioData.length);
		if (trackIndex === 0) {
			trackIndex = audioData.length - 1;
		} else {
			trackIndex -= 1;
		}
        console.log('prev song track index',trackIndex);
        loadTrack();
		playPauseAudio();
	};
